...
''' adapted from https://github.com/quarbby'''
...
class Vision(QMainWindow):
    ...
    #===========================Tab widget creation ==========================================
        ...
        #self.main_tab.addTab(self.main_frame, "Vision_filter")
    ...
    #def initUI(self):
        ...
        colorMap = QLabel()
        try:
            pixmap = QPixmap(os.getcwd() + "/huescale.jpeg")
        ...
        yellow_chk, self.yellow_chkbox, layout_yellow = self.make_data_chkbox("yellow:   ")
        green_chk, self.green_chkbox, layout_green = self.make_data_chkbox("green:    ")
        red_chk, self.red_chkbox, layout_red = self.make_data_chkbox("red:         ")
        orange_chk, self.orange_chkbox, layout_orange = self.make_data_chkbox("orange:  ")

        self.changeParamsBtn = QPushButton("Change")
        self.changeParamsBtn.clicked.connect(self.changeParamsBtnHandler)

        lo_params_layout = QHBoxLayout()
        lo_params_layout.addLayout(layout_lo_h)
        lo_params_layout.addLayout(layout_lo_s)
        lo_params_layout.addLayout(layout_lo_v)
        Hi_params_layout = QHBoxLayout()
        Hi_params_layout.addLayout(layout_hi_h)
        Hi_params_layout.addLayout(layout_hi_s)
        Hi_params_layout.addLayout(layout_hi_v)  
        
        clrseg_params_layout = QVBoxLayout()
        clrseg_params_layout.addLayout(layout_yellow)
        clrseg_params_layout.addLayout(layout_red)
        clrseg_params_layout.addLayout(layout_green)
        clrseg_params_layout.addLayout(layout_orange)
        
        Vparams_layout = QVBoxLayout()
        Vparams_layout.addLayout(lo_params_layout)
        Vparams_layout.addLayout(Hi_params_layout)
        Vparams_layout.addWidget(self.changeParamsBtn)
        
        params_layout = QHBoxLayout()
        params_layout.addLayout(Vparams_layout)
        params_layout.addLayout(clrseg_params_layout)
    ...
    def initSub(self):
        self.cam_sub = rospy.Subscriber("/image_raw", Image, self.cam_callback) 
        self.filter_sub = rospy.Subscriber("/image_filtered", Image, self.filter_callback) #/fucked is ./roscv_test
    ...
    def updateParamsMap(self, loH, hiH, loS, hiS, loV, hiV):
        ...
        self.hist.setParams(self.params)
        client = dynamic_reconfigure.client.Client('bucket_vision')
        colparams = { 'hueLow' : loH , 'satLow' : loS , 'valLow' : loV , 'hueHigh' : hiH , 'satHigh' : hiS , 'valHigh' : hiV}
        config = client.update_configuration(colparams)
